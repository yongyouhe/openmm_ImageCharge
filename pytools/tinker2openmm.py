...
'''
NOTE:
    support all the local axis types of multipoles.
'''
import argparse
import shlex
...
class TinkerOpenmmConverter:
    ...
    def write(self, xml_file, pdb_file, name_prefix, pdb_conect, xml_bond):
        ...
        # leetspeak ;) )
        if name_prefix == None:
            self.atom_type_prefix = str('')
            self.atom_class_prefix = str('')
        else:
        ...
        self.xml_make_residue(xml_bond)
        ...
        xmlstr = minidom.parseString(
            tostring(self.ff)
        ).toprettyxml(indent="   ") # indent指定缩进字符串,默认为制表器
        ...
        with open(pdb_file, 'w') as f:
            ...
            for atm in self.xyz_atoms:
                ...
                f.write('{0: >3}'.format(""))
                f.write('{0: >8}'.format(str(atm.x)[0:7]))    # x
                f.write('{0: >8}'.format(str(atm.y)[0:7]))    # y
                f.write('{0: >8}'.format(str(atm.z)[0:7]))    # z
            ...
            # loop over bonds:
            if pdb_conect == 'single':
                for bnd in self.xyz_bonds:
                    f.write('{0: <6}'.format("CONECT"))
                    f.write('{0: >5}'.format(bnd.bondfrom))
                    f.write('{0: >5}'.format(bnd.bondto))
                    f.write("\n")
            elif pdb_conect == 'origin':
                bondfrom = self.xyz_bonds[0].bondfrom
                for bnd in self.xyz_bonds:
                    if bondfrom==bnd.bondfrom and self.xyz_bonds.index(bnd)!=0:
                        f.write('{0: >5}'.format(bnd.bondto))
                    elif self.xyz_bonds.index(bnd) == 0:
                        f.write('{0: <6}'.format("CONECT"))
                        f.write('{0: >5}'.format(bnd.bondfrom))
                        f.write('{0: >5}'.format(bnd.bondto))
                    else:
    ...
    def parse_prm(self, prm_file):
        ...
        with open(prm_file, "r") as f:
            ...
            for line in f:
                ...
                elif split_line[0] == "polar-12-intra":
                    self.polar_12_intra = split_line[1]
                elif split_line[0] == "polar-13-intra":
                    self.polar_13_intra = split_line[1]
                elif split_line[0] == "polar-15-intra":
                    self.polar_15_intra = split_line[1]
                ...
                elif split_line[0] == "SOLUTE":
                    print("WARNING: 'SOLUTE' records not implemented!")
    ...
    def parse_prm_multipole(self, split_line, f):
        ...
        if len(split_line) == 5:
            ...
            raise Exception("Wrong number of parameters." + split_line)
    ...
    def xml_make_residue(self, xml_bond):
        ...
        residue.set("name", self.resname[0:3])
        ...
        existing_bonds = defaultdict(list)

        # add bonds to residue:
        # (note that OpenMM has zero-based index in residue entry)
        for b in self.xyz_bonds:

            if (b.bondfrom in existing_bonds) and (b.bondto in existing_bonds[b.bondfrom]):
                continue

            if b.bondto in existing_bonds and b.bondfrom in existing_bonds[b.bondto]:
                continue
            ...
            if xml_bond == 'number':
                ...
                bond.set("atomName1", self.xyz_atoms[int(b.bondfrom)-1].name)
                bond.set("atomName2", self.xyz_atoms[int(b.bondto)-1].name)

            # add to bond dictionary:
            existing_bonds[b.bondfrom].append(b.bondto)
            existing_bonds[b.bondto].append(b.bondfrom)
    ...
    def xml_make_amoeba_multipole_force(self):
        ...
        # conversion factors between Tinker and OpenMM:
        bohr = 0.52917720859 # bohr radius, unit of Angstrom
        openmm_dipole_fac = 0.05291772          # bohr to nm
        openmm_quadrupole_fac = 0.0009334284    # TODO 0.01*bohr*bohr/3.0
        ...
        if hasattr(self, "polar14Intra"):
            multipole_force.set("polar14Intra", self.polar_14_intra)
        if hasattr(self, "polar12Intra"):
            multipole_force.set("polar12Intra", self.polar_12_intra)
        if hasattr(self, "polar13Intra"):
            multipole_force.set("polar13Intra", self.polar_13_intra)
        if hasattr(self, "polar15Intra"):
            multipole_force.set("polar15Intra", self.polar_15_intra)
        ...
        for elem in self.prm_multipole_records:
            ...
            if len(elem.ky) != 0:
                mp.set("ky", self.atom_type_prefix + str(elem.ky))
            mp.set("c0", elem.c_0)
...
def main(args):
    ...
    tp.parse(str(args.input_xyz) + ".xyz", str(args.input_prm) + ".prm")

    # write converted data to XML file:
    tp.write(
        str(args.resname) + ".xml", str(args.resname) + ".pdb",
        args.name_prefix, args.pdb_conect, args.xml_bond
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="?",
        const="residue",
        help="output file name for both xml and pdb files.",
        default="residue"
    )
    parser.add_argument(
        "-input_xyz",
        required=True,
        help="XYZ file name to input."
    )
    parser.add_argument(
        "-input_prm",
        required=True,
        help="PRM file name to input."
    )
    parser.add_argument(
        "-name_prefix",
        type=int,
    ...
    parser.add_argument(
        "-pdb_conect",
        choices=['origin', 'single'],
        help="Choose which style of CONECT in pdb file. origin: write all bonds of the atom in one line; single: write one bond one line.",
        default="origin"
    )
    parser.add_argument(
        "-xml_bond",
        choices=['name', 'number'],
        help="Choose which style of Bond in xml file. name: use atom name; number: use atom number in residue.",
        default="name"
    )
