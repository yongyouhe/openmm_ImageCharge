...
ONE_4PI_EPS0 = 138.935456  # 1/(4*pi*epsilon0)*e^2*Na unit is KJ/mol
...
class SlabCorrection(mm.CustomCVForce):
    ...
    def __init__(self, system, volume):
        ...
        nbondedForce = [f for f in [system.getForce(i) for i in range(system.getNumForces())]
                        if type(f) == mm.NonbondedForce][0]
        cvdipole = mm.CustomExternalForce('q*z')
        cvdipole.addPerParticleParameter('q')
        for i in range(system.getNumParticles()):
            #(q, sig, eps) = nbondedForce.getParticleParameters(i)
            q = nbondedForce.getParticleParameters(i)[0].value_in_unit(unit.elementary_charge)
            idxres = cvdipole.addParticle(i, [q])

        cvforce = mm.CustomCVForce("twopioverV*(muz)^2")
        cvforce.addCollectiveVariable("muz",cvdipole)
        cvforce.addGlobalParameter("twopioverV",ONE_4PI_EPS0*2*np.pi/volume)
        self.cvforceid = system.addForce(cvforce)

    def getAddforceID(self):
        return self.cvforceid

    def removeAddforce(self, system):
        system.removeForce(self.cvforceid)
